      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-fade-in-up delay-200">
          {aboutData.map((item, index) => (
            <Card
              key={index}
              className="shadow-lg rounded-xl p-8 bg-[#F5F0F0] hover:shadow-xl transition-shadow duration-300"
            >
              <div
                className={`w-20 h-20 flex items-center justify-center rounded-full ${item.color} mx-auto mb-6 transition-transform hover:scale-110`}
              >
                {item.icon}
              </div>
